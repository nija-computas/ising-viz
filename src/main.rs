use std::{sync::atomic::{AtomicI8, Ordering}, thread, time::Instant};

use nannou::{image::{DynamicImage, ImageBuffer, Luma}, prelude::*};
...
fn initialize_spin_lattice_atomic(dim: (usize, usize)) -> Array2<AtomicI8> {
    Array2::random(dim, Uniform::new(0., 1.)).mapv(|a| if a > 0.5 { AtomicI8::new(1) } else { AtomicI8::new(-1) })
}
...
fn metropolis_step(lattice: &mut Array2<i8>, beta: f64) {
    ...
    for (x, y, r) in izip!(x_values, y_values, r_values).take(m * n) {
        let right = lattice[[(x + 1) % n, y]];
        let down = lattice[[x, (y + 1) % m]];
        let left = lattice[[x.saturating_sub(1) % n, y]];
        let up = lattice[[x, y.saturating_sub(1) % m]];
...
struct Model {
    lattice: Array2<AtomicI8>
}

fn model(_app: &App) -> Model {
    _app.new_window().size(N as u32, N as u32).view(view).build().unwrap();
    Model {
        lattice: initialize_spin_lattice_atomic((N, N))
    }
}
...
fn update(_app: &App, _model: &mut Model, _update: Update) {
    ...
    metropolis_step_atomic(&mut _model.lattice, beta);
...
fn view(_app: &App, _model: &Model, frame: Frame){
    let draw = _app.draw();
    let vec: Vec<u8> = _model.lattice.iter().map(|x| u8::try_from(255*(x.load(Ordering::Relaxed) as i16 + 1)/2).unwrap()).collect();
    // let vec: Vec<u8> = _model.lattice.clone().into_raw_vec().iter().map(|x| u8::try_from(255*(*x as i16 + 1)/2).unwrap()).collect();
    let image: ImageBuffer<Luma<u8>, Vec<u8>> = ImageBuffer::from_raw(_model.lattice.shape()[0] as u32, _model.lattice.shape()[1] as u32, vec).unwrap();
